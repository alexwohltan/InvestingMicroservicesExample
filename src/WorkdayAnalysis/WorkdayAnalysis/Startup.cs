...
public class Startup
{
    ...
    public virtual IServiceProvider ConfigureServices(IServiceCollection services)
    {
        ...
        {
            services.AddDbContext<WorkdayAnalysisDbContext>(options =>
            options.UseSqlServer(
                Configuration.GetConnectionString("WorkdayAnalysisDatabase"))
            .UseLazyLoadingProxies());

            services.AddSwaggerGen(c =>
            {
                c.SwaggerDoc("v1", new Microsoft.OpenApi.Models.OpenApiInfo { Title = "Workday Analysis API", Version = "v1" });
            });
        }
        else
        {
